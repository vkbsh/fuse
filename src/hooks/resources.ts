...
export const useTokenInfo = (vaultAddress: Address) => {
  ...
  const isAllFetched =
    meta.every((m) => m.data) && price.every((p) => p.isFetched);
  const isFetched =
    meta.every((m) => m.isFetched) && price.every((p) => p.isFetched);
  const allPrices = price.every((p) => !!p.data);

  const data = meta
    .map((m, i) => {
      if (!m.data || !price[i] || !tokens[i]) return null;
  ...
  const transactions = txs
    ?.map((tx) => {
      const tokenMeta = tokensMeta.find(
        (m) => m.data?.address === tx?.message?.mintAddress,
      );
